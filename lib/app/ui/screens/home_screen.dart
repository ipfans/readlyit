...
class HomeScreen extends ConsumerWidget {
            ...
            children: [
              TextField(
                ...
                decoration: InputDecoration(
                  hintText:
                      AppLocalizations.of(dialogContext)!.enterArticleUrlHint,
                ),
              ...
              TextField(
                ...
                decoration: InputDecoration(
                  hintText:
                      AppLocalizations.of(dialogContext)!.enterArticleTitleHint,
                ),
              ),
            ],
          ),
              ...
              onPressed: () {
                ...
                if (url.isNotEmpty) {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text(
                        AppLocalizations.of(context)!.errorUrlCannotBeEmpty,
                      ),
                    ),
                  );
                }
      ...
      builder: (BuildContext dialogContext) {
        return AlertDialog(
          ...
          content: Text(
            AppLocalizations.of(
              dialogContext,
            )!.confirmDeleteArticleContent(article.title),
          ),
          actions: <Widget>[
            TextButton(
              child: Text(AppLocalizations.of(dialogContext)!.buttonCancel),
  ...
  void _initiatePocketImport(BuildContext context, WidgetRef ref) async {
    // Make it async
  ...
  void _initiatePocketImport(BuildContext context, WidgetRef ref) async { // Make it async
    ...
    scaffoldMessenger.showSnackBar(
      const SnackBar(
        content: Text('Connecting to Pocket...'),
    ...
    if (errorMessage != null) {
      scaffoldMessenger.showSnackBar(
        SnackBar(
          content: Text('Pocket Connection Failed: $errorMessage'),
        ), // Replace with localized string
      );
    } else {
      ...
      scaffoldMessenger.showSnackBar(
        const SnackBar(
          content: Text('Please authorize with Pocket in your browser.'),
        ), // Replace with localized string
      );
    }
  }
    ...
    return Scaffold(
      appBar: custom_widgets.CustomAppBar(
        titleText:
            AppLocalizations.of(
              context,
            )!.homeScreenTitle, // Localized AppBar title
        onPocketImport:
            () => _initiatePocketImport(context, ref), // Pass the callback
      ),
      ...
      body: articlesAsyncValue.when(
        data: (articles) {
          if (articles.isEmpty) {
            return Center(
              child: Text(AppLocalizations.of(context)!.articlesListEmpty),
            );
          }
            ...
            builder: (context, constraints) {
                ...
                return Card(
                  ...
                  child: ListTile(
                    leading: Checkbox(
                      ...
                      onChanged: (bool? newValue) {
                        if (newValue != null) {
                          ref
                              .read(articlesListProvider.notifier)
                              .toggleReadStatus(article.id, newValue);
                        }
                      },
                    ),
                    title: Text(
                      ...
                      maxLines:
                          isGridView
                              ? 3
                              : 2, // More lines for title in grid view
                    ...
                    trailing: IconButton(
                      ...
                      tooltip:
                          AppLocalizations.of(context)!.tooltipDeleteArticle,
                      onPressed:
                          () => _confirmDeleteArticle(context, ref, article),
                    ),
                    onTap: () {
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder:
                              (context) => ArticleViewScreen(article: article),
                        ),
                      );
                    },
                  ),
              ...
              if (constraints.maxWidth < 600) {
                ...
                if (constraints.maxWidth < 350)
                  crossAxisCount = 1; // Safety for very narrow desktop windows
                ...
                return GridView.builder(
                  ...
                  gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
                    ...
                    childAspectRatio:
                        2.8, // Adjust this value based on desired item height/width ratio
                    crossAxisSpacing:
              ...
              child: Column(
                ...
                children: [
                  Text(
                    AppLocalizations.of(
                      context,
                    )!.errorLoadingArticles(error.toString()),
                  ),
                  ElevatedButton(
                    onPressed:
